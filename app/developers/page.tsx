...
const developers = [
  { name: 'Nikhil Karthik', position: 'Web Developer', image: '/devteam/nikhil.webp' },
  { name: 'Pratik Pakhale', position: 'Web Developer', image: '/devteam/pakhale.webp' },
  { name: 'Ashith Shetty', position: 'Web Developer', image: '/devteam/ashith.webp' },
  { name: 'Chinmay Shewale', position: 'Web Developer', image: '/devteam/charlie.webp' },
];

export default function DeveloperPage() {
  return (
    <div className="bg-black min-h-screen flex flex-col items-center justify-center px-4">
      <h1 className="text-4xl text-white mb-8 text-center">Meet the Development Team</h1>
        ...
        {developers.map((developer) => (
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-lg">
              ...
              <div className="absolute top-0 left-0 w-full h-full glitch group-hover:opacity-100"></div> {/* Glitch effect */}
            </div>

            {/* Developer Details */}
            <div className="bg-black p-4">
              <h2 className="text-xl text-white font-semibold">{developer.name}</h2> {/* Bold name */}
              <p className="text-md text-gray-400">{developer.position}</p> {/* Position text */}
            </div>
          </div>
        ))}
      </div>
